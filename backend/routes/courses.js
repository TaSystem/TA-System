...
router.get('/course-open', (req, res) => {
  db.query(
    'SELECT * FROM courses  WHERE year IN (SELECT DISTINCT(year) FROM datestudy  WHERE status = 1) AND term IN (SELECT DISTINCT(term) FROM datestudy  WHERE status = 1) ORDER BY courseID',
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
...
router.get('/sec/:id', (req, res) => {
  ...
  db.query(
    'SELECT C.sec_D,C.sec_P FROM courses as C,teacherapplyta as TA WHERE  C.id =Ta.courseID AND TA.id = ?',
    id,
    (err, result) => {
      if (err) {
        console.log(err);
...
router.get('/student', (req, res) => {
  db.query(
    'SELECT C.id,C.courseID,C.title,C.sec_D,C.day_D,C.start_D,C.end_D,C.sec_P,C.day_P,C.start_P,C.end_P,C.major,C.teacher FROM courses as C,teacherapplyta as TA WHERE C.id=TA.courseID AND TA.status = 5 AND C.year IN (SELECT DISTINCT(year) FROM datestudy WHERE status = 1) AND C.term IN (SELECT DISTINCT(term) FROM datestudy WHERE status = 1 ) ORDER BY C.courseID',
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
...
router.get('/student-apply-success/:email', (req, res) => {
  const email = req.params.email;
  db.query(
    'SELECT C.courseID,C.title,C.level,C.major,C.teacher,C.sec_D,C.sec_P,C.day_D,C.start_D,C.end_D,C.day_P,C.start_P,C.end_P,C.number_D,C.number_P,C.numberReal,A.id as AID,A.status,A.hrperweek FROM courses AS C,studentapplyta AS A ,users AS U WHERE C.id = A.courseID AND A.status = 3 AND U.id=A.userID AND C.year IN (SELECT DISTINCT(year) FROM datestudy WHERE status = 1) AND C.term IN (SELECT DISTINCT(term) FROM datestudy WHERE status = 1) AND U.email = ?',
    email,
    (err, result) => {
      if (err) {
        console.log(err);
...
router.get('/student-request-success/:id', (req, res) => {
  const id = req.params.id;
  db.query(
    'SELECT C.courseID,C.title,C.level,C.major,C.teacher,C.sec_D,C.sec_P,C.day_D,C.start_D,C.end_D,C.day_P,C.start_P,C.end_P,C.number_D,C.number_P,C.numberReal,A.id as AID,A.status,A.hrperweek FROM courses AS C,studentapplyta AS A ,users AS U WHERE C.id = A.courseID AND A.status = 3 AND U.id=A.userID AND C.year IN (SELECT DISTINCT(year) FROM datestudy WHERE status = 1) AND C.term IN (SELECT DISTINCT(term) FROM datestudy WHERE status = 1) AND U.id = ?',
    id,
    (err, result) => {
      if (err) {
        console.log(err);
...
router.post('/teacher-reply', (req, res) => {
  //แสดงรายวิชาที่อาจารย์ยื่นขอ sa ของหัวหน้าภาค
  let email = req.body.email;
  let status = req.body.status;
  let major = null;
...
router.post('/delete', (req, res) => {
  //multiple delete ลบรายวิชาที่เพิ่มจากเจ้าหน้าที่
  ...
  if (!year) {
    ...
    if (term) {
      if (major) {
        ...
        item = [year, term];
        message =
          'ลบวิชาทั้งหมดวิชาของ' + ' ปีการศึกษา ' + year + ' ภาคเรียน ' + term;
...
router.post('/single-upload', (req, res) => {
  //อัดโหลดไฟล์เดียว เพิ่มรายวิชาด้วย excel.csv
      ...
      csvtojson()
        ...
        .then((source) => {
          ...
          for (let i = 0; i < source.length; i++) {
            ...
            if (courseID == '03602312') {
              //03602312 วิศวกรรมอุสาหการและระบบเบื้องต้น 1 คน 2ชม.
              console.log('03602312 วิชาอุตขอได้พิเศษ');
              success++;
              numberTa = 1;
            } else if (courseID == '03602362') {
              //03602362 ระบบการผลิตอัตโนมัติ  1 คน 4ชม.
              ...
              console.log(
                courseID + ' ' + sec_D + ' ' + sec_P + 'สัมมนาไม่รับนิสิต'
              ); //สัมมนา
              unsucess++;
              status = false;
            } else if (number_P && number_D) {
              ...
              if (number_D < 40) {
                console.log(
                  courseID +
                    ...
                    ' ' +
                    sec_P +
                    'บรรยายและปฎิบัติมีน้อยกว่า 40 คน'
                );
                unsucess++;
                status = false;
                ...
                console.log(
                  courseID +
                    ...
                    ' ' +
                    sec_P +
                    'บรรยายและปฎิบัติมีมากกว่า 60 คน'
                );
                success++;
                numberTa = 2;
              }
              ...
              if (number_D < 40) {
                console.log(
                  courseID +
                    ...
                    ' ' +
                    sec_P +
                    'บรรยายมีน้อยกว่า 40 คน'
                );
                unsucess++;
                status = false;
              } else if (number_D >= 40 && number_D <= 60) {
                console.log(
                  courseID + ' ' + sec_D + ' ' + sec_P + 'บรรยายมี 40-60  คน'
                );
                success++;
                numberTa = 1;
              } else {
                console.log(
                  courseID + ' ' + sec_D + ' ' + sec_P + 'บรรยายมีมากกว่า 60 คน'
                );
                success++;
                numberTa = 2;
              }
              ...
              console.log(
                courseID +
                  ...
                  sec_P +
                  ' ' +
                  'ไม่เข้าเงื่อนไขใดๆไม่มีสิทธิ์ขอ'
              );
              unsucess++;
              status = false;
            }
...
router.post('/multiple-upload', (req, res) => {
  //ไม่ได้ใช้ อัพโหลดไฟล์หลายไฟล์พร้อมกัน
  let file = req.files.filename;
...
router.put('/updateNumber', (req, res) => {
  //เพิ่มจำนวนนิสิตที่ลงทะเบียนจริงๆ
  let id = req.body.id,
    numberReal = req.body.numberReal;

  db.query(
    'UPDATE courses SET numberReal = ? WHERE id = ?',
    [numberReal, id],
    (err, result) => {
      if (err) throw err;
      else {
        db.query(
          'SELECT C.id as CID,C.courseID,C.title,C.level,C.major,C.teacher,C.sec_D,C.number_D,C.sec_P,C.number_P,C.day_D,C.day_P,C.start_D,C.end_D,C.start_P,C.end_P,C.numberReal,A.id as AID,A.number1,A.number2,A.status,A.noteapply,U.name_email FROM courses AS C,teacherapplyta AS A,users AS U WHERE C.id = A.courseID AND A.userID=U.id AND A.status = 4',
          (err, result) => {
            if (err) throw err;
            else {
              res.send(result);
...
router.put('/update', (req, res) => {
  ...
  db.query(
    'UPDATE users SET courseID = ?,courseYear = ?,title = ?,credit = ?,unit_D = ?,hr_D = ?,sec_D = ?,day_D = ?,start_D = ?,dat1 = ?,end_D = ?,room_D = ?,major_D=? ,number_D = ?,unit_P = ?,hr_P =?,sec_P =?,day_P = ?,start_P =?,dat2 =?,end_P =?,room_P = ?,major_P =?,number_P = ?,teacher =?,codeTeacher = ?,subjectBefore =?,testTime =?,central_M =?,decentral_M =?,central_F =?,decentral_F =?,note =? WHERE id = ?',
    [
      courseID,
      courseYear,
      title,
