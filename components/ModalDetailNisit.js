...
export default function ModalDetailNisit(props) {
  ...
  const downloadCard = async () => {
    Axios({
      ...
      link.setAttribute(
        'download',
        `${props.val.idStudent}` + '-fileCardStudent.pdf'
  ...
  const downloadBookBank = async () => {
    Axios({
      ...
      link.setAttribute(
        'download',
        `${props.val.idStudent}` + '-fileBookBank.pdf'
      ); //or any other extension
      document.body.appendChild(link);
      link.click();
    });
  ...
  const bankData = () => {
    if (props.role == 1) {
      return (
        <div>
          <p>
            ชิ่อธนาคาร: {props.val.nameBank} &emsp; เลขบัญชี: {props.val.idBank}
          </p>
      ...
      <div className="modal-dialog modal-dialog-centered">
        <div className="modal-content">
          ...
          <div className="modal-body">
            ...
            <p>
              ชื่อ-สกุล: {props.val.name} {props.val.lastname} &emsp; รหัสนิสิต:{' '}
              {props.val.idStudent}{' '}
            </p>
            <p>
              ระดับ: {props.val.lvl} &emsp; ภาควิชา:{props.val.department}
            </p>
            <p>
              อีเมลล์: {props.val.email} &emsp; เบอร์โทรศัพท์: {props.val.tel}
            </p>
            {bankData()}
          </div>
